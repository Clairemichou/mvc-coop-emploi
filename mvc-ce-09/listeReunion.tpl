        ...
        {foreach  from=$listeReunion item=$reunion}
            <article class="col-sm-12 maincontent">
                <header class="page-header">
                    <h4 class="page-title">
                        {$reunion->reu_dat|date_format : "%A %d %m %Y"|capitalize}<br>
                        {$reunion->lie_nom} - {$reunion->lie_cpo} {$reunion->lie_vil}
                    </h4>
                </header>
                <p>{$reunion->reu_pre}</p>
                <p>La réunion débutera à {$reunion->reu_heu|date_format:"%Hh%M"} pour une durée de
                    {$reunion->reu_dur|date_format:"%Hh%M"} environ</p>
                <form method="post" action="ficheInscription.php">
                    ...
                    <input type="hidden" name="rappel"
                           value="{$reunion->reu_dat|date_format : "%A %d %m %Y"|capitalize} à {$reunion->reu_heu|date_format:"%Hh%M"}
                    {$reunion->lie_nom} - {$reunion->lie_cpo} {$reunion->lie_vil}">

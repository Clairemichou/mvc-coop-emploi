...
//        var_dump($this->valeurs);
    ...
    public function genererAffichageInscription($valeurs)
    {
        $this->valeurs = $valeurs;
        ...
        $this->tpl->assign('listeInscriptions', $this->valeurs);

        $this->tpl->assign('reunion', $this->valeurs->fetch());

        $this->tpl->display('mod_reunion/vue/inscriptionVue.tpl');
    }

    public function genererAffichageInscriptionPDF($valeurs)
    {
        $pdf = new PDF();
// Titres des colonnes
        $header = array(utf8_decode('N°'), utf8_decode('Prénom'), 'Nom', 'Emargement');
// Chargement des données
        $pdf->SetFont('Arial', '', 14);
        $pdf->AddPage();
        $pdf->FancyTable($header, $valeurs);
        $pdf->Output();
    }
